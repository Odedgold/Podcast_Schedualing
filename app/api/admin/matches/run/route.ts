...
function slotToUtc(slot: WeeklySlot, tz: string): UtcSlot {
  ...
  const base = DateTime.fromObject({ year: refDay.year, month: refDay.month, day: refDay.day, hour: sh, minute: sm, second: 0 }, { zone: tz }).toUTC()
  const baseEnd = DateTime.fromObject({ year: refDay.year, month: refDay.month, day: refDay.day, hour: eh, minute: em, second: 0 }, { zone: tz }).toUTC()
